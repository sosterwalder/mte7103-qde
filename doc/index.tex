...
\end{tabular}
\end{center}\footnotetext[1]{\label{org7dc34b7}sven.osterwalder@students.bfh.ch}\footnotetext[2]{\label{org0b118b7}claude.fuhrer@bfh.ch}\footnotetext[3]{\label{org41ac812}eric.dubuis@comet.ch}
...
\label{sec:org1406fa1}
\subsection{Code}
\label{sec:orga0e976a}

\subsubsection{{\bfseries\sffamily TODO} Principles}
\label{sec:orga56cefa}
...
As a starting point, we shall implement the class \myverb{MainWindow}
representing the main window.

\subsection{{\bfseries\sffamily TODO} Main window}
\label{sec:org290352f}
...
implementation. The Qt documentation states about \myverb{parent}:
...
run. The \myverb{data} method accesses a property of the scene view model
when getting the second column, the number of nodes a scene contains:
\myverb{graph_node_count}. As the scene view model is only a placeholder at the
moment, it is necessary to implement that property first. As the name says, the
property \myverb{graph_node_count} returns the number of graph nodes a scene view
...
\myverb{graph_node_count} method.
...
The method \myverb{graph_node_count} then simply returns the length of the
...
column names correctly, the \myverb{headerData} method has to be implemented.
...
to the header data. The implementation of the \myverb{headerData} is shown in
...
the selection model provides the \myverb{selectionChanged} signal. The
selection model is inherent in the data model of the \myverb{QTreeView}. For
being able to use the signal, the \myverb{setModel} method of the tree view
...
selected.
For more flexibility, the slot \myverb{on_tree_item_selected} will be
...
valid for the widget, the \myverb{WidgetShortcut} shortcut context is used.
...
\myverb{triggered} signal is connected with the slot \myverb{on_new_tree_item}
and \myverb{on_tree_item_removed} respectively.

The implementation of the addition and removal of tree items within the scene
graph view is shown in listings \ref{lst:gui-scene-graph-slots-02} and
\ref{lst:gui-scene-graph-slots-02}.
...
\myverb{Application} class is spawned. As stated before, this class
is a central aspect of the application as it connects the various layers of the
architecture. The \myverb{Application} class spawns the main window, creates
...
\end{minted}
\caption{\label{lst:gui-scene-graph-slots-03}
...
\myverb{tree_item_added} or \myverb{tree_item_removed}, is emitted.
...
As stated before, the \myverb{Application} class acts as a connection between
...
As the scene graph controller inherits from \myverb{QAbstractItemModel} the
corresponding methods, \myverb{insertRows} and \myverb{removeRows}, have
to be implemented. First, let us implement the slots as they are very easy to
implement. Implementing \myverb{on_tree_item_added} is straightforward: the
\myverb{insertRows} method is called by providing the row, the count and
...
The implementation of \myverb{on_tree_item_added} is analogous: the
\myverb{removeRows} method is called by providing the row, the count and
...
\myverb{beginInsertRows} by providing the index of the parent item, the
...
then being ended by calling \myverb{endInsertRows}. Finally the view widget
is being told to redraw itself by emitting the \myverb{layoutChanged} signal
and the \myverb{scene_added} signal is emitting the newly created domain
...
\myverb{beginRemoveRows} by providing the index of the parent item, the
current row and the last row of insertion (which is the current row plus the
count minus one).
...
node. The transaction is then being ended by calling \myverb{endRemoveRows}.
Finally, again the view widget is being told to redraw itself by emitting the
\myverb{layoutChanged} signal and the \myverb{scene_removed} signal is
...
which may be as simple as using Python's \myverb{print} function. Using the
\myverb{print} function may allow printing something immediately, but it
...
\subsubsection{{\bfseries\sffamily TODO} Logging}
\label{sec:orgb8063ad}

As logging is a very central and basic functionality, the module is placed in
the \myverb{foundation} layer.
...
\myverb{basicConfig} whenever something is logged for the first time. This
...
\myverb{info.log}, and a error file handler, which logs errors to a file
named \myverb{error.log}. The default level is set to debug and all handlers
are used.
...
The decorator will be implemented as a method named \myverb{with_logger} in
the \myverb{common} module. All, that this method does is to set the logger
name to the name of the module it is in combined with his own name. It then
attaches a property named \myverb{logger} to the class that calls the method.
...
The implementation of the \myverb{with_logger} method allows the usage of the
...
The class \texttt{SomeClass} gets annotated by the \texttt{with\_logger} decorator from the \texttt{common} module. All of the class's methods are then able to use the \texttt{logger} property as can be seen in method \texttt{some\_method}.}
\end{listing}
...
necessary. First, the \myverb{common} module needs to be imported.
...
Second, the class needs the \myverb{with_logger} decorator.
...
The \texttt{with\_logger} decorator is added to the scene graph view class's decorators.}
\end{listing}
