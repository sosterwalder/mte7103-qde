...
#+ATTR_LaTeX: :options fontsize=\footnotesize,linenos,bgcolor=bashcodebg
#+CAPTION:    Definition of the body of the =scene= module, which is in the =gui_domain= layer, {{{code-ref(guidomain-scene-body)}}}.
...
#+CAPTION:    The method =setup_connections= being defined by setting the scene graph widget's model, {{{code-ref(app-application-methods-setup-connections)}}}.
#+BEGIN_SRC python
<app-application-methods-setup-connections>=
